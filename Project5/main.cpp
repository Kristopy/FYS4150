...
void Diffusion_1D_FE(double dx){
  ...
  double dt = 0.1*dx*dx;      // Deciding step length for time based on stability condition
  ...
  if (Terminal == "Y"){
    cout << u(0,span::all) << endl;
  }

  if (Choice == "Y"){
    //std::cout << std::setprecision(9) << u << '\n';
    ofile << setiosflags(ios::showpoint | ios::uppercase);
    ofile << fixed <<setprecision(12) << u << endl;
    ofile.close();
  }
  return;
}
