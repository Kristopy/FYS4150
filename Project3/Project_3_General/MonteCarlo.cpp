...
void OutputToFileMC(double a, double b, int n, string outfilename){
    ...
    ofile << "|    N:    |  MonteCarlo BF:  | CPU-time MonteCarlo BF: |   Relative error BF:   |  MonteCarlo IS:    |  CPU-time MonteCarlo IS: |  Relative error IS:   |   Exact value:   |" << endl;
    ofile << "|          |                  |                         |                        |                    |                          |                       |                  |" << endl;

    for(int N = 2; N <= n ; N++){
            long int h = pow(10,N);

            for (long int p = 5; p <= 50; p++){
                long int Values = h*p*0.020;
