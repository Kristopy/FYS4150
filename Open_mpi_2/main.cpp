...
// Reactangle rule and numerical integration using MPI send and Receive 2
...
int main (int nargs, char* args[])
{
    ...
    // MPI initializations
    MPI_Init (&nargs, &args);
    MPI_Comm_size (MPI_COMM_WORLD, &numprocs); MPI_Comm_rank (MPI_COMM_WORLD, &my_rank);
    ...
    // Every process sets up its contribution to the integral local_sum = 0.;
    ...
    if (my_rank == 0) {
         MPI_Status status; rectangle_sum = local_sum;
